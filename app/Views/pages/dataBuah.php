...
<div class="container">
  <div class="tittle">
    <h1 class="label-tambah-data"><b id="headerTitle">Data Buah</b></h1>
  ...
  <div class="row mt-3">
    ...
    <div class="col-6 d-flex justify-content-center">
      <!-- Tombol Sudah Panen -->
      <a href="<?= base_url(); ?>/dataBuah?filter=sudah_panen" 
      ...
      <!-- Tombol Akan Panen -->
      <a href="<?= base_url(); ?>/dataBuah?filter=akan_panen" 
    ...
    <table id="tablehome" class="table table-bordered table-hover ">
      <thead class="table-dark align-middle ">
        <tr class="align-middle">
          ...
          <th scope="col">
            <?php if (!empty($data_buah)) : ?>
                <?php 
                    $hasNull = false;
                    $hasNonNull = false;
                    
                    foreach ($data_buah as $item) {
        ...
        <?php $i = 1; ?>
          <?php foreach ($data_buah as $buah) : ?>
            <?php
              ...
              $waktuPrakiraanPanen = isset($buah['waktu_prakiraan_panen']) ? strtotime($buah['waktu_prakiraan_panen']) : null;
              $hariIni = strtotime(date('Y-m-d'));

              // Jika waktu_panen NULL dan waktu_prakiraan_panen adalah hari ini atau sudah lewat, beri warna merah
              $highlight = (is_null($buah['waktu_panen']) && $waktuPrakiraanPanen !== null && $waktuPrakiraanPanen <= $hariIni) ? 'table-color' : '';
          ?>
          <tr class="align-middle table-light">
            ...
            <td><?= $buah['jumlah_tanam'] . ' ' . $buah['kategori_tumbuhan'] . ''; ?></td>

            <!-- Waktu Panen / Prakiraan Panen -->
            <td class="<?= (is_null($buah['waktu_panen']) && $waktuPrakiraanPanen !== null && $waktuPrakiraanPanen <= $hariIni) ? 'table-color' : ''; ?>">
              <?= $buah['waktu_panen'] ?? ($buah['waktu_prakiraan_panen'] ?? '-'); ?>
            </td>

            <td>
              <?php if (!isset($buah['waktu_panen']) || $buah['waktu_panen'] == null) : ?>
                  <!-- Ikon Panen -->
                  <a href="<?= base_url(); ?>/dataBuah/dataPanenBuah/<?= $buah['id_buah']; ?>" class="text-success mx-2" title="Panen">
                      <i class="fas fa-seedling"></i>
                  </a>
                  <!-- Ikon Edit -->
                  <a href="<?= base_url(); ?>/dataBuah/editBuah/<?= $buah['id_buah']; ?>" class="text-warning mx-2" title="Edit">
              ...
              <?php else : ?>
                  ...
                  <a href="<?= base_url(); ?>/dataBuah/dataPanenBuah/<?= $buah['id_buah']; ?>" class="text-warning mx-2" title="Edit Data Panen">
                      <i class="fas fa-edit"></i>
                  </a>
                  <!-- Ikon Delete -->
                  <?php if (isset($buah['id_buah'])) : ?>
...
<script>
    $(document).ready(function () {
      ...
      function updateHeaderTitle(filter) {
          let headerText = 'Data Buah'; // Default
          if (filter === 'sudah_panen') {
              headerText = 'Data Buah Sudah Panen';
          } else if (filter === 'akan_panen') {
              headerText = 'Data Buah Akan Panen';
          }
