...
<div class="container">
  <div class="tittle">
    <h1 class="label-tambah-data"><b id="headerTitle">Data Tanaman Obat</b></h1>
  ...
  <div class="row mt-3">
    ...
    <div class="col-6 d-flex justify-content-center">
      <!-- Tombol Sudah Panen -->
      <a href="<?= base_url(); ?>/dataTanamanObat?filter=sudah_panen" 
      ...
      <!-- Tombol Akan Panen -->
      <a href="<?= base_url(); ?>/dataTanamanObat?filter=akan_panen" 
    ...
    <table id="tablehome" class="table table-bordered table-hover ">
      <thead class="table-dark align-middle ">
        <tr class="align-middle">
          ...
          <th scope="col">
            <?php if (!empty($data_tanaman_obat)) : ?>
                <?php 
                    $hasNull = false;
                    $hasNonNull = false;
                    
                    foreach ($data_tanaman_obat as $item) {
        ...
        <?php $i = 1; ?>
          <?php foreach ($data_tanaman_obat as $obat) : ?>
            <?php
                ...
                $waktuPrakiraanPanen = isset($obat['waktu_prakiraan_panen']) ? strtotime($obat['waktu_prakiraan_panen']) : null;
                $hariIni = strtotime(date('Y-m-d'));

                // Jika waktu_panen NULL dan waktu_prakiraan_panen adalah hari ini atau sudah lewat, beri warna merah
                $highlight = (is_null($obat['waktu_panen']) && $waktuPrakiraanPanen !== null && $waktuPrakiraanPanen <= $hariIni) ? 'table-color' : '';
          ?>
          <tr class="align-middle table-light">
            ...
            <td><?= $obat['jumlah_tanam'] . ' ' . $obat['kategori_tumbuhan'] . ''; ?></td>

            <!-- Waktu Panen / Prakiraan Panen -->
            <td class="<?= (is_null($obat['waktu_panen']) && $waktuPrakiraanPanen !== null && $waktuPrakiraanPanen <= $hariIni) ? 'table-color' : ''; ?>">
              <?= $obat['waktu_panen'] ?? ($obat['waktu_prakiraan_panen'] ?? '-'); ?>
            </td>

            <td>
              <?php if (!isset($obat['waktu_panen']) || $obat['waktu_panen'] == null) : ?>
                  <!-- Ikon Panen -->
                  <a href="<?= base_url(); ?>/dataTanamanObat/dataPanenTanamanObat/<?= $obat['id_tanaman_obat']; ?>" class="text-success mx-2" title="Panen">
                      <i class="fas fa-seedling"></i>
                  </a>
                  <!-- Ikon Edit -->
                  <a href="<?= base_url(); ?>/dataTanamanObat/editDataTanamanObat/<?= $obat['id_tanaman_obat']; ?>" class="text-warning mx-2" title="Edit">
              ...
              <?php else : ?>
                  ...
                  <a href="<?= base_url(); ?>/dataTanamanObat/dataPanenTanamanObat/<?= $obat['id_tanaman_obat']; ?>" class="text-warning mx-2" title="Edit Data Panen">
                      <i class="fas fa-edit"></i>
                  </a>
                  <!-- Ikon Delete -->
                  <?php if (isset($obat['id_tanaman_obat'])) : ?>
...
<script>
    $(document).ready(function () {
      ...
      function updateHeaderTitle(filter) {
          let headerText = 'Data Tanaman Obat'; // Default
          if (filter === 'sudah_panen') {
              headerText = 'Data Tanaman Obat Sudah Panen';
          } else if (filter === 'akan_panen') {
              headerText = 'Data Tanaman Obat Akan Panen';
          }
